%% @author Korney
%% @doc @todo Add description to programmerLogger.


-module(programmerLogger).
-behaviour(gen_event).
...
-spec handle_event(Event :: term(), State :: term()) -> Result when
	...
	Module2 :: atom().
%% ====================================================================
handle_event({task},State)->
	error_logger:info_msg("Programmer: received task ~n",[]),
	{ok, State};

handle_event({bugFixed},State)->
	error_logger:info_msg("Programmer: bug fixed~n",[]),
	{ok, State};
